...
def is_owner(user_id: int) -> bool:
    """Returns True if user is the bot owner (set via OWNER_ID env var)."""
    from bot.config import OWNER_ID
    return OWNER_ID != 0 and user_id == OWNER_ID


def is_admin(user_id: int) -> bool:
    """Sync check: owner only. Use is_any_admin() to also check DB admins."""
    from bot.config import OWNER_ID
    return user_id == OWNER_ID


async def is_any_admin(user_id: int) -> bool:
    """Async check: owner OR any DB-added admin."""
    from bot.config import OWNER_ID
