    ...
    try:
        ...
        try:
            ...
            try:
                await bot.send_message(
                    user_id,
                    "⏱️ OTP timeout. The session did not receive any login code in 5 minutes.\n\n"
...
async def process_zip_upload(zip_bytes: bytes, country_code: str, two_fa_password: str, added_by: int) -> Dict:
    db = get_db()
    ...
    with zipfile.ZipFile(io.BytesIO(zip_bytes)) as zf:
        for name in zf.namelist():
            ...
            try:
                ...
                if not phone:
                    failed += 1
                    errors.append(f"{name}: unauthorized or no phone")
                    continue

                existing = await db.sessions.find_one({"phone_number": phone})
                if existing:
                    failed += 1
                    errors.append(f"{name}: already exists ({phone})")
                    continue
    ...
    return {"success": True, "added": added, "failed": failed, "errors": errors[:5]}
