...
from bot.config import (
    UPI_ID, UPI_NAME, USDT_ADDRESS, USDT_NETWORK, ADMIN_IDS,
    AUTO_APPROVE_UPI, AUTO_APPROVE_CRYPTO, CRYPTO_EXCHANGE_RATE
)
...
async def cb_deposit_upi(callback: CallbackQuery, state: FSMContext):
    ...
    mode = "⚡ *Instant Credit*" if AUTO_APPROVE_UPI else "🔍 Manual Verification"
...
async def process_upi_amount(message: Message, state: FSMContext):
    try:
        ...
        if amount < 10:
            await message.answer("❌ Minimum ₹10. Enter a valid amount:", reply_markup=cancel_deposit_kb())
            return
        if amount > 50000:
    ...
    caption = (
        ...
        f"━━━━━━━━━━━━━━━━━━━━━━━━\n"
        f"{'⚡ *Balance credited instantly!*' if AUTO_APPROVE_UPI else '🔍 Admin will verify shortly.'}"
    )
...
async def process_upi_utr(message: Message, state: FSMContext):
    ...
    if AUTO_APPROVE_UPI:
        ...
        await log_event(user_id, "deposit_approved", {"method": "upi", "amount": amount, "utr": utr, "request_id": request_id})

        await processing_msg.delete()
        await message.answer(
            f"✅ *UPI Deposit Approved!*\n"
            f"━━━━━━━━━━━━━━━━━━━━━━━━\n\n"
            f"💰 Amount: `₹{amount:.2f}` added!\n"
...
async def cb_deposit_crypto(callback: CallbackQuery, state: FSMContext):
    ...
    mode = "⚡ *Blockchain Auto-Verify*" if AUTO_APPROVE_CRYPTO else "🔍 Manual Verification"
    await callback.message.edit_text(
        f"💎 *USDT Deposit*\n"
        f"━━━━━━━━━━━━━━━━━━━━━━━━\n"
        f"Status: {mode}\n\n"
        f"Enter USDT amount:\n"
        f"_Minimum: $1 | Rate: 1 USDT ≈ ₹{CRYPTO_EXCHANGE_RATE:.0f}_",
        reply_markup=cancel_deposit_kb(), parse_mode="Markdown"
    )
    await callback.answer()
...
async def process_crypto_amount(message: Message, state: FSMContext):
    ...
    inr_value = amount * CRYPTO_EXCHANGE_RATE
    ...
    await message.answer(
        ...
        f"━━━━━━━━━━━━━━━━━━━━━━━━\n"
        f"✅ After sending, paste *Transaction Hash* (TxID):",
        reply_markup=cancel_deposit_kb(), parse_mode="Markdown"
    )
...
async def process_crypto_txhash(message: Message, state: FSMContext):
    ...
    inr_value = data.get("crypto_inr", amount * CRYPTO_EXCHANGE_RATE)
    request_id = generate_request_id()
    user_id = message.from_user.id
    db = get_db()
