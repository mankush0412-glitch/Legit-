...
async def cb_admin_settings(callback: CallbackQuery):
    ...
    min_deposit = await get_setting("min_deposit_amount", 10.0)
...
async def cb_set_min_deposit(callback: CallbackQuery, state: FSMContext):
    if not await is_any_admin(callback.from_user.id):
        return
    current = await get_setting("min_deposit_amount", 10.0)
    await state.set_state(AdminStates.set_min_deposit)
    await callback.message.edit_text(
        f"💰 *Set Minimum Deposit Amount*\n\n"
        f"Current: `₹{current:.0f}`\n\n"
        f"Enter new minimum deposit amount in ₹:\n"
        f"_(e.g. `50` means users must deposit at least ₹50)_",
        reply_markup=cancel_admin_kb(), parse_mode="Markdown"
    )
    await callback.answer()
...
async def process_set_min_deposit(message: Message, state: FSMContext):
    if not await is_any_admin(message.from_user.id):
        return
    try:
        amount = float(message.text.strip().replace("₹", "").replace(",", ""))
        if amount < 1:
            await message.answer("❌ Minimum must be at least ₹1:")
            return
        await set_setting("min_deposit_amount", amount)
        await state.clear()
        await message.answer(
            f"✅ *Minimum Deposit Updated!*\n\n"
            f"Users must now deposit at least `₹{amount:.0f}`",
            reply_markup=admin_panel_kb(is_owner=is_owner(message.from_user.id)),
            parse_mode="Markdown"
        )
    except ValueError:
        await message.answer("❌ Invalid. Enter a number like `50`:")
