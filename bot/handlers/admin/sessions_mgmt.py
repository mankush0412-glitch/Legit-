...
async def process_zip_password(message: Message, state: FSMContext):
    ...
    if password.lower() in ("none", "skip", "-", "no"):
        password = ""
    # Pass from_callback=False so we always use answer() on user messages
    await _finish_zip_upload(message, state, password=password, from_callback=False)


async def _finish_zip_upload(message_or_msg, state: FSMContext, password: str, from_callback: bool = False):
    data = await state.get_data()
    country_code = data.get("zip_country", "")
    country_name = data.get("zip_country_name", country_code)
    country_flag = data.get("zip_country_flag", "")
    ...
    # from_callback=True → edit the bot message (Skip button was pressed)
    # from_callback=False → answer() to user's typed message
    if from_callback:
        try:
